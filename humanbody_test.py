...
def detect_and_color_splash(model, image_path=None, video_path=None):
    ...
    #dataset.load_person(dataset_dir, subset)
    #dataset.prepare()
    ...
    if image_path:
        ...
        # Read image
        image = skimage.io.imread(args.image, as_grey=True)
        if image.ndim != 3:
        ...
        # Visualize
        visualize.display_instances(image, r['rois'], r['masks'], r['class_ids'], "person", r['scores'])
        ...
        skimage.io.imsave(file_name, visualize.display_instances(image, r['rois'], r['masks'], r['class_ids'], "person", r['scores']))
    elif video_path:
        import cv2
        # Video capture
        ...
        while success:
            ...
            if success:
                ...
                # Color splash
                #splash = color_splash(image, r['masks'])
                splash = visualize.display_instances(image, r['rois'], r['masks'], r['class_ids'], "person", r['scores'])
...
if __name__ == '__main__':
    ...
    if args.weights.lower() == "coco":
        ...
        # number of classes, grayscale image excludes the first Conv layer
    ...
    if args.command == "train":
        ...
        # Validation dataset
        dataset_val = PersonDataset()
        dataset_val.load_person('./humanbody', 'val')
        dataset_val.prepare()
        ...
        '''for image_id in image_ids:
            ...
            recall = utils.compute_recall(r["rois"], gt_bbox, 0.5)
            print(recall)      
        ...
        for image_id in image_ids:
            ...
            image, image_meta, gt_class_id, gt_bbox, gt_mask = \
                modellib.load_image_gt(dataset_val, config,
                               image_id, use_mini_mask=False)
                 # Load and save all ground truth
            if count1 == 0:
        ...
        ap = utils.compute_ap_range(save_box, save_class, save_mask,
                         save_roi, save_id, save_score, save_m)
        print(ap)
 
         # Draw PR curve
        plt.plot(recalls, precisions, 'b', label='PRcurve')
        plt.title('precision-recall curve')
        plt.xlabel('Recall')
        plt.ylabel('Precision')
        plt.legend()
